...
def send_message(sentiment, text, customer_name, customer_email, message_id):
    ...
    message_data = {
        ...
        "sentiment": sentiment  # Pre-set sentiment to ensure consistent data
    }
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            print(f"✅ Added {sentiment} message from {customer_name}: {text[:50]}...")
            return True
        else:
...
def populate_database():
    ...
    for sentiment, messages in sample_messages.items():
        print(f"\n📧 Adding {len(messages)} {sentiment} email messages...")
        
        for i, message_text in enumerate(messages):
            customer_name, customer_email = customers[i]
            message_id = f"{sentiment}_{int(time.time())}_{i+1}"
            
            if send_message(sentiment, message_text, customer_name, customer_email, message_id):
    ...
    if successful_messages > 0:
        ...
        print("\n📈 Sentiment breakdown:")
        print(f"   • Angry messages: {len(sample_messages['angry'])}")
        print(f"   • Happy messages: {len(sample_messages['happy'])}")
        print(f"   • Confused messages: {len(sample_messages['confused'])}")
        print(f"   • Neutral messages: {len(sample_messages['neutral'])}")
